...
def test_login_api_success(user_account, unauthorized_api_client, mocker):
    ...
    mocked_validate = mocker.patch(
        f"{LOGIN_SERIALIZER_PATH}.validate", return_value={"user": user}
    )
    mocked_response = Response(status=status.HTTP_204_NO_CONTENT)
    mocked_login = mocker.patch(
        "{{ cookiecutter.project_slug }}.apps.accounts.services.login.LoginService.login",
...
@pytest.mark.django_db
def test_login_api_wrong_credentials_failure(
    user_account, unauthorized_api_client, mocker
):
    mocked_validate = mocker.patch(
        f"{LOGIN_SERIALIZER_PATH}.validate",
        side_effect=[ValidationError(gettext("Incorrect email or password."))],
    )
    ...
    assert response.data == {
        "non_field_errors": [gettext("Incorrect email or password.")]
    }
    mocked_validate.assert_called_once_with(data)
