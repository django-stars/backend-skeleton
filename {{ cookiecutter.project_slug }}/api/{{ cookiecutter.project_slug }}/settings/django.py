...
INTERNAL_IPS = env.list(
    "{{ cookiecutter.project_slug | upper() }}_INTERNAL_IPS", default=[]
)

ALLOWED_HOSTS = env.list(
    "{{ cookiecutter.project_slug | upper() }}_ALLOWED_HOSTS", default=[]
)
...
DATABASES = {
    "default": env.db(
        "{{ cookiecutter.project_slug | upper() }}_DATABASE_URL",
...
SECURE_BROWSER_XSS_FILTER = env.bool(
    "{{ cookiecutter.project_slug | upper() }}_SECURE_BROWSER_XSS_FILTER", default=True
)
...
SESSION_COOKIE_HTTPONLY = env.bool(
    "{{ cookiecutter.project_slug | upper() }}_SESSION_COOKIE_HTTPONLY", default=True
)
SESSION_COOKIE_SECURE = env.bool(
    "{{ cookiecutter.project_slug | upper() }}_SESSION_COOKIE_SECURE", default=True
)
CSRF_COOKIE_SECURE = env.bool(
    "{{ cookiecutter.project_slug | upper() }}_CSRF_COOKIE_SECURE", default=True
)
X_FRAME_OPTIONS = env.str(
    "{{ cookiecutter.project_slug | upper() }}_X_FRAME_OPTIONS", default="SAMEORIGIN"
)
SECURE_HSTS_SECONDS = env.int(
    "{{ cookiecutter.project_slug | upper() }}_SECURE_HSTS_SECONDS", default=31536000
)  # 1 year
...
STATIC_URL = env.str(
    "{{ cookiecutter.project_slug | upper() }}_STATIC_URL", default="/s/"
)
STATIC_ROOT = env.str(
    "{{ cookiecutter.project_slug | upper() }}_STATIC_ROOT",
    default=rel("..", "..", "public", "static"),
)

MEDIA_URL = env.str(
    "{{ cookiecutter.project_slug | upper() }}_MEDIA_URL", default="/m/"
)
MEDIA_ROOT = env.str(
    "{{ cookiecutter.project_slug | upper() }}_MEDIA_ROOT",
    rel("..", "..", "public", "media"),
)
FILE_UPLOAD_PERMISSIONS = 0o644

EMAIL_BACKEND = env.str(
    "{{ cookiecutter.project_slug | upper() }}_EMAIL_BACKEND",
